...
var AuthorizerAccessToken = function (authorizer_access_token, expireTime) {
  if (!(this instanceof AuthorizerAccessToken)) {
    return new AuthorizerAccessToken(authorizer_access_token, expireTime);
  }
  this.accessToken = authorizer_access_token;
...
/**
 * 根据 component_appid, component_access_token, authorizer_appid, authorizer_refresh_token 创建API的构造函数。
 * 如需跨进程跨机器进行操作Wechat API（依赖access token），access token需要进行全局维护
 * 使用策略如下：
 *
 * 1. 调用用户传入的获取token的异步方法，获得token之后使用
 * 2. 使用appid/authorizer_refresh_token获取token。并调用用户传入的保存token方法保存
 *
 * Examples:
 * ```
 * var API = require('wechat-open-service');
 * var apiAgent = new API.Agent('component_appid','component_access_token','authorizer_appid','authorizer_refresh_token');
 * ```
 * 以上即可满足单进程使用。
 ...
 * @param {String} component_access_token 第三方平台令牌
 * @param {String} authorizer_appid 代调用的客户公众号appid
 * @param {String} authorizer_refresh_token 在授权后获得的授权方的刷新令牌
 * @param {Function} getToken 可选的。获取全局token对象的方法，多进程模式部署时需在意
 * @param {Function} saveToken 可选的。保存全局token对象的方法，多进程模式部署时需在意
 */
var API = function (component_appid, component_access_token, authorizer_appid, authorizer_refresh_token, getToken, saveToken) {
...
API.prototype.request = function (url, opts, callback) {
  ...
  //替换掉url中的access_token成为authorizer_access_token
  url = url.replace("access_token","authorizer_access_token");
  urllib.request(url, options, callback);
};


/*!
 * 根据创建API时传入的 appid, providersecret获取suite access token
 ...
 * Examples:
 * ```
 * api.getAuthorizerAccessToken(callback);
 * ```
 * Callback:
 *
...
API.prototype.getAuthorizerAccessToken = function (callback) {

  var url = this.prefix + 'component/api_authorizer_token?component_access_token=' + this.component_access_token;
  var data = {
  ...
  this.request(url, postJSON(data), wrapper(function (err, data) {
    ...
    var token = AuthorizerAccessToken(data.authorizer_access_token, expireTime);
...
API.prototype.preRequest = function (method, args, retryed) {
  ...
  that.getToken(function (err, token) {
    ...
    var accessToken;
    // 有token并且token有效直接调用
    if (token && (accessToken = AuthorizerAccessToken(token.accessToken, token.expireTime)).isValid()) {
      // 暂时保存token
      ...
      that.getAuthorizerAccessToken(function (err, token) {
        // 如遇错误，通过回调函数传出
        if (err) {
          return callback(err);
        }
...
API.prototype.getLatestToken = function (callback) {
  ...
  that.getToken(function (err, token) {
    ...
    // 有token
    if (token) {
      callback(null, AuthorizerAccessToken(token));
    } else {
      that.getAuthorizerAccessToken(callback);
    }
  });
};
...
API.AuthorizerAccessToken = AuthorizerAccessToken;
